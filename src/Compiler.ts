import type { ASTNode, Metadata } from "./types.js";
...
export default class Compiler {
    ...
    private compileInlineChildren(node: ASTNode, isDarkTheme: boolean): string {
        ...
        switch (node.type) {
            ...
            case "TABLE": {
                const rows = (node.children || [])
                    .map((row) =>
                        this.compileNode(row, isDarkTheme, indentLevel + 1),
                    )
                    .join("\n");
                return `${indent}<table>\n${rows}\n${indent}</table>`;
            }
            ...
            case "TABLE_ROW": {
                const cells = (node.children || [])
                    .map((cell) =>
                        this.compileNode(cell, isDarkTheme, indentLevel + 1),
                    )
                    .join("");
...
${indent}</div>`;
            ...
            case "LIST": {
                ...
                const items = node.children
                    .map((child) =>
                        this.compileNode(child, isDarkTheme, indentLevel + 1),
                    )
                    .join("\n");
                return `${indent}<ul>\n${items}\n${indent}</ul>`;
            }
            ...
            case "LISTITEM": {
                ...
                if (sublist) {
                    ...
                    const compiledSublist = this.compileNode(
                        sublist,
                        isDarkTheme,
                        indentLevel + 1,
                    );
            ...
            default:
                this.warnings.push(
                    `Unknown node type during compilation: ${node.type}`,
                );
                return "";
        }
    }
