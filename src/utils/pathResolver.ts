...
export const ALLOWED_FORMATS = ["html", "pdf", "image"] as const;
export type OutputFormat = (typeof ALLOWED_FORMATS)[number];
...
export function resolveCliPaths(
    ...
    if (rawOutput && isOutputFormat(rawOutput.toLowerCase())) {
        ...
        if (ext === ".pdf") candidateFormat = "pdf";
        else if ([".png", ".jpg", ".jpeg"].includes(ext))
            candidateFormat = "image";
        else if (ext === ".html") candidateFormat = "html";
    }

    // 2. Validate format using Type Guard
    const isValidFormat = isOutputFormat(candidateFormat);
    const outputFormat: OutputFormat = isValidFormat
        ? (candidateFormat as OutputFormat)
        : "html";
